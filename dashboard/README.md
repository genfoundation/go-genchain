## Go Genchain Dashboard

The dashboard is a data visualizer integrated into ggen, intended to collect and visualize useful information of an Genchain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-genchain` repo), these need to be installed first:

```
$ (cd dashboard/assets && yarn install && yarn flow)
```

Normally the dashboard assets are bundled into Ggen via `go-bindata` to avoid external dependencies. Rebuilding Ggen after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ ggen --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Ggen, run `go generate`:
