...
func (db *Dashboard) collectData() {
	...
	var (
		...
		prevDiskRead       = metrics.DefaultRegistry.Get("gen/db/chaindata/disk/read").(metrics.Meter).Count()
		prevDiskWrite      = metrics.DefaultRegistry.Get("gen/db/chaindata/disk/write").(metrics.Meter).Count()
	...
	for {
		...
		case <-time.After(db.config.Refresh):
			...
			var (
				...
				curDiskRead       = metrics.DefaultRegistry.Get("gen/db/chaindata/disk/read").(metrics.Meter).Count()
				curDiskWrite      = metrics.DefaultRegistry.Get("gen/db/chaindata/disk/write").(metrics.Meter).Count()
