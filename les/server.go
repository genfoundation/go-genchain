...
import (
	...
	"github.com/genchain/go-genchain/les/flowcontrol"
...
func NewLesServer(gen *gen.Genchain, config *gen.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(gen.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, gen.EventMux(), gen.Engine(), newPeerSet(), gen.BlockChain(), gen.TxPool(), gen.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(gen.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(gen.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(gen.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(gen.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(gen.ChainDb())
	return srv, nil
}
