...
import (
	...
	rpc "github.com/genchain/go-genchain/rpc"
)
...
func New(ctx *node.ServiceContext, config *gen.Config) (*LightEthereum, error) {
	chainDb, err := gen.CreateDB(ctx, config, "lightchaindata")
	if err != nil {
		return nil, err
	}
	...
	leth := &LightEthereum{
		...
		engine:           gen.CreateConsensusEngine(ctx, &config.Ethash, chainConfig, chainDb),
		shutdownChan:     make(chan bool),
		networkId:        config.NetworkId,
		bloomRequests:    make(chan chan *bloombits.Retrieval),
		bloomIndexer:     gen.NewBloomIndexer(chainDb, light.BloomTrieFrequency),
...
// APIs returns the collection of RPC services the genchain package offers.
