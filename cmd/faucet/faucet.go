...
// faucet is a Genc faucet backed by a light client.
package main
...
import (
	...
	"github.com/genchain/go-genchain/ethclient"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Genchain protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Genchain and the faucet")
)

var (
	genc = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Genchain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Genchain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Genchain protocol stack
	client *ethclient.Client   // Client connection to the Genchain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Genchain light client protocol
...
// close terminates the Genchain connection and tears down the faucet.
...
// apiHandler handles requests for Genc grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, genc),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Genchain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/genchain/go-genchain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), genc)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, genc)
...
// the username, avatar URL and Genchain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Genchain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Genchain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Genchain address and profile URL.
...
// returning the username, avatar URL and Genchain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Genchain address,
