...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 genc` in value to maximum `1 genc` per 24h period
...
implemented for `ggen`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
