...
import (
	...
	"github.com/genchain/go-genchain/cmd/utils"
	"github.com/genchain/go-genchain/common"
	"github.com/genchain/go-genchain/console"
	"github.com/genchain/go-genchain/crypto"
	"github.com/genchain/go-genchain/ethclient"
	"github.com/genchain/go-genchain/internal/debug"
	...
	"github.com/genchain/go-genchain/swarm"
	bzzapi "github.com/genchain/go-genchain/swarm/api"
	swarmmetrics "github.com/genchain/go-genchain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the Genchain API provider to use to settle SWAP payments",
...
// This init function sets defaults so cmd/swarm can run alongside ggen.
func init() {
...
var app = utils.NewApp(gitCommit, "Genchain Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-genchain Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.genchain/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.genchain/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.genchain/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.genchain/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//ggen only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for ggen and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the genchain node
	...
	//register BZZ as node.Service in the genchain node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	//register within the genchain node
