...
FROM genchain/client-go:alltools-latest
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Genchain listener port":       strconv.Itoa(info.node.port),
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			port:     infos.portmap[infos.envvars["GEN_PORT"]+"/tcp"],
			ethstats: infos.envvars["GEN_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
