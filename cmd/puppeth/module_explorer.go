...
RUN \
  echo '(cd ../gen-net-intelligence-api && pm2 start /ethstats.json)' >  explorer.sh && \
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.genchain
    environment:
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.genchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
