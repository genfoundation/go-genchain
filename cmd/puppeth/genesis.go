...
// newCppEthereumGenesisSpec converts a go-genchain genesis block into a Parity specific
// chain specification format.
func newCppEthereumGenesisSpec(network string, genesis *core.Genesis) (*cppEthereumGenesisSpec, error) {
	// Only ethash is currently supported between go-genchain and cpp-genchain
	...
	spec.Params.BlockReward = (*hexutil.Big)(ethash.GenBlockReward)
...
// newParityChainSpec converts a go-genchain genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-genchain and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Parity's format
	...
	spec.Engine.Ethash.Params.BlockReward = (*hexutil.Big)(ethash.GenBlockReward)
	...
	spec.Engine.Ethash.Params.EIP649Reward = (*hexutil.Big)(ethash.GenBlockReward)
	...
	spec.Genesis.Seal.Genchain.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Genchain.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Genchain.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// Python Genchain implementation.
...
// newPyEthereumGenesisSpec converts a go-genchain genesis block into a Parity specific
// chain specification format.
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only ethash is currently supported between go-genchain and pyethereum
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
