...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("gen", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("gen", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "gen_echo", i)
			if err != nil {
				if !wantError {
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-genchain-test-ipc-%d-%d", os.Getpid(), rand.Int63())
