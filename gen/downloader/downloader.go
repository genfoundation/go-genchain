...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [gen/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [gen/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [gen/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [gen/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [gen/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [gen/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [gen/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [gen/63] Channel receiving inbound node state data
...
// these are zero.
func (d *Downloader) Progress() genchain.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "gen", p.version, "head", hash, "td", td, "mode", d.mode)
