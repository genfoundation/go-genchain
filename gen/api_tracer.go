...
import (
	...
	"github.com/genchain/go-genchain/gen/tracers"
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.gen.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.gen.blockchain.CurrentBlock()
	default:
		from = api.gen.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.gen.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.gen.blockchain.CurrentBlock()
	default:
		to = api.gen.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabase(api.gen.ChainDb())

	if number := start.NumberU64(); number > 0 {
		start = api.gen.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.gen.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.gen.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.gen.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.gen.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.gen.blockchain.CurrentBlock()
	default:
		block = api.gen.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.gen.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.gen.engine.VerifyHeader(api.gen.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.gen.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.gen.blockchain, nil)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.gen.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.gen.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.gen.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.gen.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.gen.blockchain, nil)
		if idx == txIndex {
