...
func New(gen Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		gen:      gen,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, gen, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(gen.BlockChain(), engine))
	go miner.update()
