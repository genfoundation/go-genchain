...
// Whisper represents a dark communication interface through the Genchain
...
type Whisper struct {
	...
	expirations map[uint32]mapset.Set //*set.SetNonTS  // Message expiration pool
...
// New creates a Whisper client ready to communicate through the Genchain P2P network.
func New(cfg *Config) *Whisper {
	...
	whisper := &Whisper{
		...
		expirations:   make(map[uint32]mapset.Set),//*set.SetNonTS),
...
func (whisper *Whisper) add(envelope *Envelope, isP2P bool) (bool, error) {
	...
	if !alreadyCached {
		...
		if whisper.expirations[envelope.Expiry] == nil {
			whisper.expirations[envelope.Expiry] = mapset.NewThreadUnsafeSet()//set.NewNonTS()
		}
