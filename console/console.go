...
import (
	...
	"github.com/genchain/go-genchain/internal/jsre"
	"github.com/genchain/go-genchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Genchain requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var gen = web3.gen; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = gen.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines gen.getBalance(gen.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Ggen instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Ggen metadata
	fmt.Fprintf(c.printer, "Welcome to the Ggen JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + gen.coinbase);
		console.log("at block: " + gen.blockNumber + " (" + new Date(1000 * gen.getBlock(gen.blockNumber).timestamp) + ")");
