...
func newTester(t *testing.T, confOverride func(*gen.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Genchain service within
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return gen.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Genchain protocol: %v", err)
	}
