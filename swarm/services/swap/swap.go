...
import (
	...
	"github.com/genchain/go-genchain/contracts/chequebook"
	"github.com/genchain/go-genchain/contracts/chequebook/contract"
	"github.com/genchain/go-genchain/core/types"
	"github.com/genchain/go-genchain/crypto"
	"github.com/genchain/go-genchain/log"
	"github.com/genchain/go-genchain/swarm/services/swap/swap"
)
...
var (
	...
	autoCashThreshold    = big.NewInt(50000000000000)  // threshold that triggers autocash (shao)
	autoDepositInterval  = 300 * time.Second           // default interval for autocash
	autoDepositThreshold = big.NewInt(50000000000000)  // threshold that triggers autodeposit (shao)
	autoDepositBuffer    = big.NewInt(100000000000000) // buffer that is surplus for fork protection etc (shao)
	buyAt                = big.NewInt(20000000000)     // maximum chunk price host is willing to pay (shao)
	sellAt               = big.NewInt(20000000000)     // minimum chunk price host requires (shao)
...
func NewSwap(local *SwapParams, remote *SwapProfile, backend chequebook.Backend, proto swap.Protocol) (self *swap.Swap, err error) {
	...
	var buy, sell string
	if self.Buys {
		buy = "purchase from peer enabled at " + remote.SellAt.String() + " shao/chunk"
	} else {
		buy = "purchase from peer disabled"
	}
	if self.Sells {
		sell = "selling to peer enabled at " + local.SellAt.String() + " shao/chunk"
	} else {
