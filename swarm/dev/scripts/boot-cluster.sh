...
#
# The cluster contains a bootnode, a ggen node and multiple swarm nodes, with
...
# static ggen configuration
GGEN_IP="192.168.33.3"
GGEN_RPC_PORT="17090"
GGEN_RPC_URL="http://${GGEN_IP}:${GGEN_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the ggen and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a ggen node with --datadir pointing at <base-dir>/ggen
# and a single, unlocked account with password "ggen"
start_geth_node() {
  local dir="${base_dir}/ggen"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ggen account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GGEN_IP}"
    --rpcport   "${GGEN_RPC_PORT}"
    --verbosity "6"
  )

  start_node "ggen" "${GGEN_IP}" "$(which ggen)" ${args[@]}
}
...
create_account() {
  ...
  ggen --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
